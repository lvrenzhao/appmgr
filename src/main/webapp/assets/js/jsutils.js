var URL_DICT = app.ctx + "/constant/dic.do";//todo 获取码表的controller接口地址
var URL_REGION = app.ctx+"/constant/childregion.do"; //todo 获取区域的controller接口地址
                ...
                if (data && data.length > 0) {
                    $(selectid).each(function() {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            html += '<option ' + (initid == data[i].CODEVALUE ? 'selected="selected" ' : '') + 'value="' + data[i].CODEVALUE + '">' + data[i].CODETEXT + '</option>'
                        }
                        $(this).html(html);
                    });
                } else {
...
/**地市和区县二级联动的下拉框组件，两个参数分别是地市下拉框和区县下拉框的选择器*/
function initRegionSelectLevel2(ds,qx) {
                ...
                success : function(data) {
                    ...
                    if (data && data.length > 0) {
                        $(qx).each(function() {
                            ...
                            var html = '<option value="">--请选择--</option>';
                            for (var i = 0; i < data.length; i++) {
                                html += '<option ' + (initid == data[i].REGIONID ? 'selected="selected" ' : '') + 'value="' + data[i].REGIONID + '">' + data[i].REGIONNAME + '</option>'
                            }
                            $(this).html(html);
                        });
                    } else {
...
function getRecent30Years() {
    var cy = new Date().getFullYear();
    var arr = [cy];
    for(var i = cy+5; i> cy-5; i-- ){
        arr.push(i);
    }
    return arr;
}
